...
1. **冷却检查** — 用户是否已在冷却期？是则静默丢弃（`event.stop_event()`，0 token 消耗）
2. **刷屏检测** — 窗口内消息数 >= 阈值？是则触发冷却（所有消息类型）
3. **重复消息检测** — 窗口内相同文本 >= 阈值？是则触发冷却（仅文本消息）
4. **正常处理** — 未触发任何规则，传递给下一个 handler / LLM
...
完全静默丢弃。不会发送给 LLM，不会消耗 token，也不会发送任何回复（私聊场景下会发送一次警告消息）。
</details>
...
<details>
<summary>Q: 图片、表情包等非文本消息会被拦截吗？</summary>

会被刷屏频率检测拦截（10秒内发5条图片也会触发冷却），但不会被重复消息检测拦截（重复检测仅针对文本内容）。
</details>

<details>
<summary>Q: 群聊中不同的人同时发消息会互相影响吗？</summary>

不会。检测是**按用户独立进行**的，每个用户有自己的追踪队列和冷却状态。A 刷屏不会影响 B 的正常消息处理。
</details>
